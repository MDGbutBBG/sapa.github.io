...
export default function PartyPosts({party,PARTIES,navigateTo,openPost}) {
    ...
    return (
        <div className="p-6 animate-fade-up h-[70vh] flex flex-col items-center justify-center px-6">
            <h1 className="text-3xl font-bold mb-6">โพสต์ทั้งหมดของพรรค {currentParty.name}</h1>
            {posts.length === 0 ? (
                    ...
                    <div className=" space-y-4 animate-fade-up">
                        {posts.map((p) => (
                            <div
                                ...
                                className="hover:-translate-y-1 transition-all cursor-pointer mt-4 p-5 w-100 bg-white rounded-3xl border border-blue-50 shadow-xs "
                            >
                            <div>
