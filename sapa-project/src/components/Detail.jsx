  ...
  return (
    <div className="max-w-xl mx-auto px-6 pt-8 space-y-4">
      ...
      <div className="animate-fade-up bg-white p-7 rounded-3xl shadow-xl"> 
        ...
        <div className="pt-2 flex flex-2">
          <div className="w-10 h-10 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-900 relative overflow-hidden">
            <img src={party.icon} alt="" className="w-full h-full object-cover"/>
          </div>
          <span className="font-medium ml-3 mt-1">{party.name}</span>
        </div>
      ...
      <div className="animate-fade-up bg-white p-5 rounded-3xl shadow-xl">
        ...
        <button onClick={() => handleCommentSubmit()} className="mt-4 bg-blue-950 text-white px-4 py-2 rounded-md flex items-center hover:cursor-pointer hover:bg-blue-700 transition gap-1"> <Send className="w-4 h-4"/>ส่งความคิดเห็น</button>
              ...
              return (
                comment.tag === 'user' ? (
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="font-medium">{comment.userName || "ผู้ใช้ทั่วไป"}</span>
                  </div>
